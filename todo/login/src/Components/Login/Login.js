...
const emailReducer = (state,action)=>{
    ...
    switch(action.type){
        ...
        case 'INPUT_EVENT' :
            return {value :action.payload.value,isValid:action.payload.value.trim().includes('@') };
        case 'INPUT_BLUR' :
...
const passwordReducer = (state,action)=>{
    ...
    switch(action.type){
        case 'INPUT_PASSWORD':
            return {value:action.payload.value,isValid:action.payload.value.trim().length > 6};
        case 'INPUT_PWD_BLUR':
...
const Login = (props) => {
    ...
    const emailChangeHandler = (event) => {
        ...
        dispatchEmail({type:"INPUT_EVENT",payload:{value:event.target.value}});
    ...
    const passwordChangeHandler = (event) => {
        ...
        dispatchPassword({type:"INPUT_PASSWORD",payload:{value:event.target.value}});
            ...
            <Card className = {classes.login}>
                <form onSubmit={submitHandler}>
                    <div className={`${classes.control} ${emailState.isValid===false ? classes.invalid : ''}`}>
                        ...
                        <input type='email' id='userEmail' value={emailState.value} onChange={emailChangeHandler} onBlur={validateEmailHandler} /> 
                    </div>
